...
- (void)viewDidLoad {
  ...
  if (!self.navigationButtonColor) {
    self.navigationButtonColor = self.navigationController.navigationBar.tintColor;
  }
  ...
  self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:self.forwardButton];
...
- (void)displayInitialViewController {
  ...
  [self.KVOController observe:viewController keyPath:[self.dataSource walkthroughViewController:self
...
- (void)cycleFromViewController:(UIViewController *)oldC
  ...
  [self transitionFromViewController:oldC toViewController:newC duration:0.25 options:0
    ...
    [self.KVOController observe:newC keyPath:[self.dataSource walkthroughViewController:self
...
- (void)displayNextViewController {
  ...
  if ([self.delegate respondsToSelector:@selector(walkthroughViewController:
    willContinueFromViewController:
    toViewController:)]) {
    [self.delegate walkthroughViewController:self willContinueFromViewController:currentViewController
...
- (FRDLivelyButton *)backButton {
  if (!_backButton) {
    _backButton = [[FRDLivelyButton alloc] initWithFrame:CGRectMake(0, 0, 36, 28)];
    [_backButton setStyle:kFRDLivelyButtonStyleCaretLeft animated:NO];
    [_backButton setOptions:@{
      kFRDLivelyButtonLineWidth : @(self.navigationButtonLineWidth),
...
- (FRDLivelyButton *)forwardButton {
  if (!_forwardButton) {
    _forwardButton = [[FRDLivelyButton alloc] initWithFrame:CGRectMake(0, 0, 36, 28)];
...
- (void)goBack {
  if (self.currentIndex == 0) {
    ...
    [self.delegate walkthroughViewController:self willGoBackFromFirstViewController:firstViewController];
  } else {
...
- (void)goForward:(FRDLivelyButton *)forwardButton {
  if (self.currentIndex == self.viewControllers.count - 1) {
    ...
    [self.delegate walkthroughViewController:self willContinueFromLastViewController:lastViewController];
    return;
  }
  ...
  if (forwardButton.buttonStyle == kFRDLivelyButtonStyleClose) {
    ...
    if ([self.delegate respondsToSelector:@selector(walkthroughViewController: couldNotContinueFromViewController:)]) {
      [self.delegate walkthroughViewController:self couldNotContinueFromViewController:currentViewController];
    }
  } else {
